...
const MARKET = {
      ...
      checks: [
        {
          label: 'Weekly structure',
          verdict: 'neutral', weight: 1.5,
          read: '⬇️ BULL → NEUTRAL, on the condition this check wrote down itself: “a re-widening print puts this straight back to neutral.” It re-widened. Weekly hist is −9.23, negative a fifth bar and EXPANDING again after the single bar of contraction that turned it bull. Weekly RSI 57.57, and price $569.41 has closed back under the weekly 9-EMA $572.14 — the second half of the configuration it flipped on. ⚠️ The 50-week $460.45 and 200-week $293.96 are untouched and far below; this is the near frame turning, not the trend breaking.',
        },
        {
          label: 'Daily trend',
          verdict: 'neutral', weight: 1.5,
          read: '⬇️ BULL → NEUTRAL. The $547–550 level still holds — a FIFTH close above it at $569.41 — and that half of this check is intact. What is not: $580 was taken on 08-07 and handed straight back on 08-10 (−2.28%, closing $569.41 against a $586.24 high), and the 50-EMA $570.36 this check called “reclaimed and held” is LOST, with price closing under it. Daily hist +4.12 has just turned to contracting and RSI 49.45 is back under 50.',
        },
        {
        ...
        },
        {
          ...
          verdict: 'neutral', weight: 1.5,
          read: '⬇️ BULL → NEUTRAL — HALF THE STACK IS BACK OVERHEAD. Only $547–550 and the mid-band $564.27 are still beneath price; the 50-day $570.36 and the $580 lid are both ABOVE the $569.41 close, so the two levels that made this check bull were given back in one session. ⚠️ What was already true above still is: price sits inside daily supply $571.35–592.01, with fresh daily supply $600.31–608.90 over that.',
        },
        {
  ...
  vol: [
    {
      symbol: 'VIX', value: '15.51', range: [13, 22], change: '📅 close 14.90 (−1.65%) — sixth session under 16 and a fresh low for the move; the week closed −6.94% and sits 7.1% over the weekly deviation-band floor 13.83',
      verdict: 'bull',
    ...
    {
      symbol: 'VXN', value: '23.04', range: [18, 34], change: '📅 close 22.82 (−4.72% on the day, −12.23% on the week) — sixth session under ≈26, sitting ON the weekly band 22.78 with the next one 18.28, still 19.9% below',
      verdict: 'neutral',
  ...
  note: '📅 Закриття 10.08. 🚦 SMH $569.41 (−2.28%) — рівень $547–550 тримається п’яту сесію, але $580, узятий уперше лише сесією раніше, віддано за один день: саме це минула записка називала умовою зміни читання. ⭐ Структурний факт сесії: SMH втратив 50-денну $570.36, а тижнева гістограма (−9.23) знову РОЗШИРЮЄТЬСЯ — сходження рамок, на якому трималась теза, розпалось. QQQ −0.30%: падіння вузьке, у напівпровідниках. Що змінить читання: денне закриття SMH під $547–550 або повернення вище $580.',
};
...
const STOCKS = [
  ...
  },
  {
    ...
    price: '$365.10', change: '📅 CLOSE $365.10 (−3.75%) — the $369 stop broke on the close, so the trade is BOOKED; the ledger scores it at T2 $403, the level it actually reached',
    signal: '📅 CLOSE 08/10 — STOPPED, AND THE FALSIFIER THIS CARD WROTE IS THE ONE THAT FIRED. Closed $365.10 (−3.75%), under the $369 stop, so the plan is closed. ⭐ The ledger books it at T2 $403 — the deepest target the trade reached — for +10.1% on the $366 fill, the way every tagged card here is scored. ⚠️ Read that honestly: it prices the T1/T2 scale-outs, NOT the whole position. Whatever ran past T2 gave back the entire $372.80–397.58 swing and came off at $365.10, under the fill. ⚠️ The stop did its job — the 50-EMA $369.92 sat on it, so losing it meant losing the daily average cluster in one move.',
    lead: { rank: 31, status: 'booked', entry: 'filled $366', stop: '$369 (close)', targets: '$390 → $403 → $419', rr: '~18:1', tagged: '$403', edge: '⛔ BOOKED — the falsifier fired exactly as written: "a daily close under $369". The close was $365.10 (−3.75%). ⭐ Scored at T2 $403 (+10.1%), the deepest level realised, per the rule that a tag survives a retrace — the board\'s model books a trade at the deepest target it reached, and TER reached two. ⚠️ THE HONEST CAVEAT, because the schema cannot express it: there is one `closed` field and one exit, so "tagged then stopped" has no representation. The +10.1% prices the T1/T2 scale-outs; the remainder stopped at $365.10, BELOW the $366 fill, so the last portion returned roughly nothing. Booking at $365.10 instead would have scored the whole trade −0.25% and erased two targets that genuinely traded — which is the erasure `tagged` exists to prevent — so the tag stands and this note carries what it leaves out. ⚠️ Note also what the card\'s own arithmetic got wrong: it predicted a stop-out "books roughly +0.8%", assuming an exit AT $369. A close-based stop exits on the close, and the close was $3.90 lower. A close stop is not a price, it is a decision taken at whatever price the session ends on.' },
    side: 'long', accent: 'blue',
    date: '2026-08-11',
  ...
  },
  {
    ...
    price: '$68.76', change: '📅 CLOSE $68.76 (−4.42%) — the $68–70 zone traded and the close is INSIDE it, so the plan is live; the ATR it was built on was never this ticker\'s',
    signal: '📅 CLOSE 08/10 — THE ZONE FILLED, AND THE ATR UNDER THE STOP TURNS OUT TO BE FICTION. Closed $68.76 (−4.42%), inside $68–70, so status goes to live. ⚠️ The stop was justified as "$63.00 is 1.08 ATR". The real ATR(14) is $6.32 (9.19%), and the extractor is blunt about the old $5.58: it sits OUTSIDE this ticker\'s entire 130-session ATR range ($6.31–11.77), so it was never ASTS on any basis. At $6.32 the $63 stop is 0.95 ATR and fails Rule B. ⭐ It moves to $60.90 — the deeper reference this card already costed out, now 1.28 ATR, under the whole $61.40–74.10 demand band. R:R ~2.7:1 → ~2.0:1. ⚠️ And the frames have turned: weekly and daily both read downtrend on the fresh bars.',
    lead: { rank: 16, status: 'live', entry: 'pullback holds $68.00–70.00', stop: '$60.90 (close)', targets: '$74.00 → $78.00 → $85.00', rr: '~2.0:1', edge: '⚠️ THE STOP WAS PRICED IN AN ATR THAT WAS NEVER THIS TICKER\'S. Last session justified $63.00 as "1.08 ATR" off ATR(14) $5.58. The computed figure is $6.32 (9.19%), and $5.58 is not a stale vintage of it — the extractor reports it OUTSIDE the whole 130-session range ($6.31–11.77), meaning no session ever produced it and it came from a different basis entirely (period, smoothing or session hours). At the true ATR, $63.00 is 0.95 and under Rule B\'s floor. ⭐ So the stop takes the deeper reference this card had already costed out — $60.90, now 1.28 ATR rather than the 1.45 quoted, sitting below the whole $61.40–74.10 daily demand band instead of at the top of it. R:R re-rates ~2.7:1 → ~2.0:1, which is the number this card predicted for that stop and is now simply the plan. ⚠️ THE ZONE ALSO FILLED: the close $68.76 is inside $68–70, so this is live rather than waiting. ⚠️ And the frames the flip rested on have turned — the fresh read has weekly −2 and daily −2, "aligned downtrend", with price under both the 50-EMA $72.39 and the 200-EMA $75.59. The long is now countertrend on its own board row; that is a materially worse trade than the one this card described, and the falsifier below is close. ⚠️ SIDE FLIPPED SHORT → LONG, AND THE REASON IS STRUCTURAL RATHER THAN A CHANGE OF MIND. The short at $69–76 was drawn when price sat far below it; price then travelled $55.74 → $71.94 in three sessions, reclaimed $62–65, and that zone now straddles the close instead of capping it. A zone price is trading INSIDE is not a rejection zone — supply has moved up to $74–76 and $78–81, and $68–70 has become the demand under price. ⭐ The frames back it: daily reversal confirmed, 4H making higher highs with improving OBV, monthly holding above its ~$58 mid-band after the bounce off $55–58. ⚠️ What is NOT repaired is the weekly — it needs acceptance over ~$81, which is also where the weekly mid-band sits, so this remains a recovery inside a correction. ⚠️ ATR(14) $6.32 is 9.19% of price — the highest-volatility name carrying a plan on this board, so size accordingly. Falsifier: daily acceptance under $61 opens $58–55.' },
  ...
  },
  {
    ...
    price: '$522.12', change: '📅 CLOSE $522.12 (−3.16%) — the session traded THROUGH $525–532 (low $521.57) and closed under it; the plan is filled, not waiting',
    signal: '📅 CLOSE 08/10 — FILLED, AND THROUGH. The low $521.57 took the whole $525–532 zone and the close $522.12 (−3.16%) sits below it. ⭐ Price is inside daily demand $519.90–552.64 and the weekly is the board\'s strongest frame at +6, a strong uptrend; the daily is a +1 range, so this is a weekly-up/daily-range pullback rather than a break. ⚠️ The stop is the weak part and always was: $492 is 0.96 ATR from the $528.50 midpoint on ATR(14) $37.88, under Rule B\'s floor, and from the close it leaves 0.80 ATR. An ordinary session reaches it. ⚠️ The structurally deeper level is the $471.12–492.17 demand band. Falsifier: a daily close under $519.90.',
    lead: { rank: 13, status: 'live', entry: 'pullback holds $525–532', stop: '$492 (close)', targets: '$555 → $590 → $615', rr: '~2.4:1', edge: '⚠️ NEAREST FILL ON THE BOARD AFTER ALOY, AND THE LEAST CONVICTION BEHIND IT — both true, and they answer different questions. The $525–532 zone sits just 0.20 ATR below price, so a fill is one ordinary session away. But every frame except the monthly reads range, conviction on the structure board is 0.0, and the weekly stays unrepaired below $575–600. Reachability is not the same as quality, and the board deliberately ranks them on different keys. ⭐ What is genuinely good: the rebound from $440–460 reclaimed $500–525 and holds above the rising weekly mid-band, and the monthly uptrend behind the parabolic run to $700–740 is intact. ⚠️ What is not: price is in the upper half of a $525–555 range rather than at an edge, and $545–555 has capped it. ⭐ The $492 stop is placed in the GAP between demand $475–490 and $505–515 — inside neither band, at exactly 1.00 ATR from the midpoint, so an ordinary test of either does not reach it. ⭐ The confirmation that matters is not the $555 break but acceptance above $615, which repairs the daily AND the weekly at once. Falsifier: daily acceptance under $505 opens $490–475.' },
    side: 'long', accent: 'red',
    date: '2026-08-11',
  ...
  },
  {
    ...
    price: '$55.06', change: '📅 CLOSE $55.06 (+5.78%) — a third straight advance, and ATR(14) expanded $3.04 → $3.59, which is what puts the $46.46 stop under the 1 ATR floor',
    signal: '📅 CLOSE 08/10 — THE STOP THIS CARD CALLED FRAGILE BROKE ITS OWN RULE, EXACTLY AS WRITTEN. Closed $55.06 (+5.78%). ⚠️ Last session set the stop at $46.46 and said it "sits EXACTLY on the 1.00 ATR floor, so any ATR expansion puts it under Rule B". ATR(14) on the fresh bars is $3.59, not $3.04 — 18% wider in one session — so $46.46 is 0.85 ATR from the $49.50 midpoint and fails. ⭐ The stop goes BACK to $45.00, and it is the re-read that changed, not the level: at $3.59 it is 1.25 ATR, not the 1.48 that got it rejected, and it is the top of daily demand $43.50–45.00 — a reference, where $45.91 is arithmetic. R:R ~2.9:1 → ~2.0:1. ⚠️ Entry $49–50 now sits 3.3% BELOW price.',
    lead: { rank: 24, status: 'wait', entry: 'pullback holds $49.00–50.00', stop: '$45.00 (close)', targets: '$52.50 → $55.50 → $58.50', rr: '~2.0:1', edge: '⚠️ THE STOP MOVED BACK TO $45.00, AND THE HONEST VERSION IS THAT LAST SESSION\'S MOVE WAS MADE ON A WRONG ATR. $46.46 was chosen because ATR(14) $3.04 put $45.00 at 1.48 ATR — "too FAR rather than too near". Computed off the fresh bars the ATR is $3.59, and on that figure $45.00 is 1.25 ATR: not far at all, while $46.46 is 0.85 and under Rule B\'s floor. So the tighter leash bought nothing and cost the reference — $45.00 is the top of daily demand $43.50–45.00, and $45.91 (the arithmetic 1.00 ATR point) sits on nothing at all. Rule A applies to stops as much as to entries: a level with no structure under it is a memory of a number. ⚠️ The cost is stated plainly, since it was the argument FOR $46.46: risk from the $49.50 midpoint goes 6.1% → 9.1% and R:R ~2.9:1 → ~2.0:1. ⭐ THE CHART CONFIRMS THE ANCHOR AND IMPROVES IT. $49.39 is a genuine drawn level on TEM, so the coincidence flagged against MP is resolved: TEM’s level is real, MP’s was spurious and has been retracted. Better still, the $49–50 zone turns out to rest on three independent references: the $49.39 pivot, the 50-EMA $50.08 at the top of the zone, and the BB mid-band $48.70 immediately beneath it. That is a stronger anchor than Rule A asks for. ⚠️ Two things in the original read were wrong and both matter. Daily RSI is 56.29 — mid-range, not stretched — so the ~73.6 belongs to the 4H alone; the case for waiting is location, not exhaustion, and a fill at $49–50 is a normal pullback rather than a capitulation. And price is ABOVE the daily BB mid-band $48.70, not below it: the "$55.8 declining mid-band" was the 200-EMA $55.54, which sits inside the $55.50–57.00 zone and is precisely why acceptance THERE, not over $52.50–54, is the confirmation that counts. ⭐ Daily MACD hist is +0.49 and has just turned positive, with OBV 345.7m. ⚠️ Read the ATR figure itself as the lesson: every stop on this board is priced in ATR units, so a stale ATR does not make a stop slightly wrong, it makes the unit wrong and every ratio quoted in it fiction. This card twice argued a stop placement to two decimal places on a number that was five sessions old. Falsifier: daily acceptance under $45.00.' },
    side: 'long',
    date: '2026-08-11',
