...
const STOCKS = [
  ...
  },
  {
    ...
    signal: '📅 CLOSE 08/06 — ⛔ LONG STOPPED OUT; THE CARD IS NOW SHORT, FILLED $224. Closed $189.88 (−13.29%) at the session low, O $205.84 · H $215.51 · L $189.17 — through the $196 stop and out the bottom of the $213–236 no-trade gap this card pre-drew. ⚠️ NEWS-driven, not earnings: the three-session post-earnings gate does NOT apply here, so an add stays live. Short filled $224: +15.2%, the best open position on the board. Stop trails to a close over $207 — above the 50-EMA $205.76, 9-EMA $200.11 and mid-band $197.51 — locking +7.6%. Targets $176.25 → $157 (200-EMA + lower band) → $145.80. ⚠️ Weekly hist −6.84, four bars minus and EXPANDING.',
    lead: { rank: 1, status: 'live', entry: 'filled $224', stop: '$207 (close)', targets: '$176.25 → $157 → $145.80', rr: '~4.6:1', edge: '⛔ THE LONG DIED AND THE SETUP FLIPPED SHORT — FILLED $224, +15.2%, THE BEST OPEN POSITION HERE. The $196 stop broke by $6.12 on a −13.29% close AT the session low ($189.88 against a $189.17 low), and price left the bottom of the $213–236 gap this card had already marked as having no edge for either side. ⭐ Every daily reference the long rested on is now overhead: the 50-EMA $205.76, the 9-EMA $200.11 and the mid-band $197.51 all sit above price, and the weekly agrees — hist −6.84, four bars negative and EXPANDING, not repairing. ⚠️ The stop is trailed, not widened: $207 clears that whole cluster at ~1.1 ATR from price and locks +7.6%, where leaving it at the old $196 would have it inside the structure the short is selling. ⚠️ Two things about that ATR, both stated rather than buried: it is approximate, because NBIS is not on the structure board and it is read off recent true ranges instead of `atrPct` — and the 06-08 bar’s own $29.82 range is roughly double the two before it, so the estimate is INFLATED by the very session it is measuring. The error runs the safe way (a stop quoted at 1.1 ATR of an inflated ATR is further out in normal-regime terms, not nearer), which is why the level stands. ⭐ And the move was NEWS-driven, not an earnings reaction, so the three-session gate that stands AXON aside to 08-11 does not reach this row: an add on a rejection into $197.51–207 is live now. ⚠️ Against: the 200-EMA $155.04 is still 18% below and monthly RSI 66.01 has not broken, so T3 is a level, not a forecast.' },
